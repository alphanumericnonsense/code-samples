...
#-------------------------------------------------------------#
# generator version of keccak and SHAKE
# modified from the above
#-------------------------------------------------------------#

def gen_Keccak(rate, capacity, inputBytes, delimitedSuffix):
    outputBytes = bytearray()
    state = bytearray([0 for i in range(200)])
    rateInBytes = rate//8
    blockSize = 0
    ...
    state[rateInBytes-1] = state[rateInBytes-1] ^ 0x80
    state = KeccakF1600(state)
    # === Squeeze out output blocks forever! ===
    while True:
        blockSize = rateInBytes
        outputBytes = state[0:blockSize]
        state = KeccakF1600(state)
        yield outputBytes

def gen_SHAKE128(inputBytes):
    """
    yields one byte
    """
    XOF = gen_Keccak(1344, 256, inputBytes, 0x1F)
    while True:
        block = next(XOF)
        for b in block:
            yield b

def gen_SHAKE256(inputBytes):
    """
    yields one byte
    """
    XOF = gen_Keccak(1088, 512, inputBytes, 0x1F)
    while True:
        block = next(XOF)
        for b in block:
            yield b
